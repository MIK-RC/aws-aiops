...
from ..utils.config_loader import get_config, AgentConfig
...
class BaseAgent(ABC):
    ...
    def invoke(self, message: str, **kwargs) -> str:
        ...
        try:
            ...
            self.record_action(
                action_type="invoke",
                description=f"Processed user request",
    ...
    async def ainvoke(self, message: str, **kwargs) -> str:
        ...
        try:
            ...
            result = await self._agent.stream_async(message, **kwargs)
            response = ""
            ...
            self.record_action(
                ...
                description=f"Processed user request (async)",
                input_summary=message,
                output_summary=response,
                success=True,
                duration_ms=duration_ms,
            ...
            self.record_action(
                action_type="async_invoke",
                description=f"Failed to process request (async)",
                input_summary=message,
                error_message=str(e),
                success=False,
                duration_ms=duration_ms,
    ...
    def get_action_summary(self) -> str:
        ...
        for i, action in enumerate(self._state.action_history, 1):
            ...
            lines.append(
                f"{i}. [{status}] {action.action_type}: {action.description}"
            )
